...
const heroLabel=p.k==="About Us"?p.t.replace(".",""):p.k;
document.querySelector("#pageHero").innerHTML=`<div><div class="crumb"><a href="index.html">Home</a> &nbsp; / &nbsp; ${p.k}</div><span class="tag orange-tag">${heroLabel}</span><h1>${p.t.replace(" "," <em>")}</em></h1></div>`;
...
if(key==="about-phakama"){
  ...
  document.querySelector(".fresh-about-intro")?.remove();
}
if(key==="our-journey"){
  document.body.classList.add("about-fresh-page");
  document.querySelector("#pageContent").innerHTML=`
...
/* Visual movement only: no page content is changed. */
const animatedItems=[...document.querySelectorAll(".fresh-about-intro,.fresh-about-section,.fresh-guidance-grid article,.fresh-cluster-grid article,.fresh-difference-rectangle li,.fresh-timeline article,.fresh-work-grid article")];
if(animatedItems.length){
  ...
  }else animatedItems.forEach(item=>item.classList.add("is-visible"));
}
